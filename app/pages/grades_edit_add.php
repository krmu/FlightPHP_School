...
if (isset($_POST['updategrade'])) {
    if (Flight::allowed_grades(false, $_POST['mark'])) {
        // We delete all grades that are not current
        Flight::db()->runQuery("update marks set deleted = 1 where module_code = ? and student_no = ?", [$module_code, $student_no]);
        // We insert new grade
...
$grade_information = Flight::db()->fetchRow("SELECT marks.id as grid,mark,last_update,uzvards,vards FROM marks left join darbinieki_user on marks.last_staff = darbinieki_user.id where module_code = ? and student_no = ? and marks.deleted = 0", array($module_code, $student_no));
        ...
        <?php
            ...
            echo "<tbody>";
                ...
                foreach ($grade_history as $grade) {
                    echo "<tr><td>" . $grade['mark'] . "</td><td>" . $grade['vards'] . " " . $grade['uzvards'] . "</td><td>" . date("d.m.Y. H:i:s", $grade['last_update']) . "</td><td>Deleted</td></tr>";
                }
            echo "</tbody>";
            echo "</table>";           
        ?>
